...
size=2*halfsize+1; 
disp(['Size of the Gaussian kernel window ' num2str(size)]);
%generate a 2-D Gaussian kernel along x direction
for i=1:size
    for j=1:size
        u=double([i-halfsize-1 j-halfsize-1]);
...
gx=conv2(single(IM),single(hx),'same');
gy=conv2(single(IM),single(hy),'same');
